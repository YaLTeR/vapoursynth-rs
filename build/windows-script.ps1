...
If ($arch -eq "i686") {
        $SUFFIX = 32
        $PYTHON_PKG = "python-3.7.5-embed-win32.zip"
} Else {
        $SUFFIX = 64
        $PYTHON_PKG = "python-3.7.5-embed-amd64.zip"
}

# Download Python embeddable and VapourSynth portable
$VS_PATH = "https://github.com/vapoursynth/vapoursynth/releases/download/R47.2"
curl -LO "https://www.python.org/ftp/python/3.7.5/$PYTHON_PKG"
curl -LO "$VS_PATH/VapourSynth$SUFFIX-Portable-R47.2.7z"

# Unzip Python embeddable and VapourSynth portable
7z x "$PYTHON_PKG" -o"$PY_DIR"
7z x "VapourSynth$SUFFIX-Portable-R47.2.7z" -o"$VS_DIR"
